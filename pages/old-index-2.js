...
export default function Shamana() {
  ...
  const slides = [
    ...
    },
    {
      ...
      description: "Join forces with other music lovers to create extraordinary playlists that transport listeners to different timelines. Get rewarded with exclusive NFTs for creating content that resonates with people who love it just as much.",
    ...
    },
    {
      ...
      description: "Join Trybes that match your unique vibe and connect with music lovers who share your taste. Discover new music, share your creations, and build lasting connections with people who get your sound.",
  ...
  const handleTimeUpdate = () => {
    if (audioRef.current) {
      ...
      if (!duration || audioRef.current.duration !== duration) {
    ...
    <>
      ...
      <style jsx global>{`
        *{margin:0;padding:0;box-sizing:border-box;}
        body{font-family:'Work Sans',sans-serif;background-color:#000;color:#fff;letter-spacing:-0.5px;max-width:1600px;margin:0 auto;padding-bottom:100px;}
        ...
        .nav a.active:after{content:'';position:absolute;bottom:-5px;left:0;width:100%;height:2px;background-color:#fff;}
        ...
        .carousel-slide-1 {
            background: 
                linear-gradient(to bottom, rgba(255,255,255,0.2) 0%, rgba(106, 17, 203, 0.8) 70%),
                url('https://res.cloudinary.com/dqhawdcol/image/upload/v1758378695/gof3qpnmxlx9sa529c0v.jpg') center/cover;
        }
        .carousel-slide-2 {
            background: 
                linear-gradient(to bottom, rgba(255,255,255,0.2) 0%, rgba(0, 180, 180, 0.8) 70%),
                url('https://res.cloudinary.com/dqhawdcol/image/upload/v1758378696/oiftniuktkros8hsnrpi.jpg') center/cover;
        }
        .carousel-slide-3 {
            background: 
                linear-gradient(to bottom, rgba(255,255,255,0.2) 0%, rgba(25, 130, 230, 0.8) 70%),
                url('https://res.cloudinary.com/dqhawdcol/image/upload/v1758378688/bqbsveb0m8l9vcirstmd.jpg') center/cover;
        }
        .carousel-content {max-width:600px;margin:0 auto;text-align:center;}
        .carousel-content h1{font-size:2.5rem;margin-bottom:15px;}
        .carousel-content p{font-size:1.1rem;margin-bottom:25px;opacity:0.9;}
        .cta-button{background:#fff;color:#6a11cb;border:none;padding:12px 30px;border-radius:30px;font-size:1rem;font-weight:600;cursor:pointer;transition:transform 0.2s;}
        .cta-button:hover{transform:scale(1.05);}
        .carousel-controls {position:absolute;bottom:20px;left:0;width:100%;display:flex;justify-content:center;z-index:10;}
        .carousel-dot {width:12px;height:12px;border-radius:50%;background:rgba(255,255,255,0.5);margin:0 5px;cursor:pointer;transition:background 0.3s ease;}
        ...
        .carousel-nav button {background:rgba(0,0,0,0.5);border:none;width:40px;height:40px;border-radius:50%;color:#fff;font-size:20px;cursor:pointer;transition:background 0.3s ease;}
        .carousel-nav button:hover {background:rgba(0,0,0,0.8);}
        .content-section {display:flex;gap:30px;margin-bottom:60px;}
        .feed-container{flex:3;background:#111;border-radius:12px;padding:20px;}
        .trybe-events-container{flex:1;display:flex;flex-direction:column;gap:30px;}
        .trybe-container{background:#111;border-radius:12px;padding:20px;}
        .events-container{background:#111;border-radius:12px;padding:20px;}
        .section-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;}
        .section-header h2{font-size:1.5rem;font-weight:600;text-transform:none;}
        .view-all{color:#6a11cb;font-weight:500;}
        .activity-item{background:#1a1a1a;border-radius:12px;padding:20px;margin-bottom:20px;}
        .activity-header{display:flex;align-items:center;gap:10px;margin-bottom:15px;}
        .creator-avatar{width:40px;height:40px;border-radius:50%;object-fit:cover;}
        .activity-info{flex:1;}
        .creator-name{font-weight:600;}
        .activity-time{font-size:0.85rem;color:#999;margin-top:3px;}
        .playlist-thumbnail{position:relative;height:180px;border-radius:8px;margin-bottom:15px;overflow:hidden;}
        .play-button{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);background:rgba(255,255,255,0.9);width:50px;height:50px;border-radius:50%;display:flex;align-items:center;justify-content:center;cursor:pointer;transition:all 0.2s;}
        .play-button:hover{background:rgba(255,255,255,1);transform:translate(-50%,-50%) scale(1.1);}
        .play-button i{color:#000;font-size:20px;}
        .playlist-info{margin-bottom:15px;}
        .playlist-name{font-size:1.2rem;font-weight:600;margin-bottom:5px;}
        .song-list{display:flex;flex-direction:column;gap:5px;margin-bottom:15px;}
        .song-item{display:flex;align-items:center;gap:10px;font-size:0.9rem;color:#ccc;}
        .song-number{color:#6a11cb;font-weight:500;width:20px;}
        .comment-section{display:flex;gap:10px;align-items:flex-start;}
        .commenter-avatar{width:30px;height:30px;border-radius:50%;object-fit:cover;}
        .comment-content{flex:1;background:#222;padding:10px 15px;border-radius:18px;}
        .commenter-name{font-weight:500;margin-bottom:3px;}
        .comment-text{font-size:0.9rem;color:#ccc;}
        .trybe-list{display:flex;flex-direction:column;gap:15px;}
        .trybe-item{display:flex;justify-content:space-between;align-items:center;padding:12px 0;border-bottom:1px solid #222;}
        .trybe-item:last-child{border-bottom:none;}
        .trybe-name{font-weight:500;}
        .follow-btn{background:transparent;border:1px solid #6a11cb;color:#6a11cb;padding:5px 12px;border-radius:15px;font-size:0.8rem;font-weight:500;cursor:pointer;transition:all 0.2s;}
        .follow-btn:hover{background:#6a11cb;color:#fff;}
        .events-list{display:flex;flex-direction:column;gap:15px;}
        .event-item{display:flex;gap:12px;padding:12px 0;border-bottom:1px solid #222;}
        .event-item:last-child{border-bottom:none;}
        .event-date{display:flex;flex-direction:column;align-items:center;justify-content:center;background:#6a11cb;border-radius:8px;min-width:50px;height:50px;padding:5px;}
        .event-day{font-size:1.2rem;font-weight:600;line-height:1;}
        .event-month{font-size:0.7rem;text-transform:uppercase;}
        .event-info{flex:1;}
        .event-title{font-weight:500;margin-bottom:3px;}
        .event-details{font-size:0.8rem;color:#ccc;}
        .join-btn{background:transparent;border:1px solid #6a11cb;color:#6a11cb;padding:5px 12px;border-radius:15px;font-size:0.8rem;font-weight:500;cursor:pointer;transition:all 0.2s;align-self:center;}
        .join-btn:hover{background:#6a11cb;color:#fff;}
        ...
        .player-info{flex:1;max-width:30%;}
        ...
        .playlist-song-item{display:flex;justify-content:space-between;align-items:center;padding:10px;border-radius:8px;cursor:pointer;transition:background 0.2s;}
        .playlist-song-item:hover{background:#2a2a2a;}
        .playlist-song-item.active{background:#6a11cb;}
        .song-info{flex:1;}
        .song-info .song-title{font-weight:500;font-size:0.9rem;margin-bottom:2px;}
        .song-info .song-artist{font-size:0.8rem;color:#ccc;}
        .song-duration{font-size:0.8rem;color:#999;min-width:40px;text-align:right;}
        ...
        .hero-carousel{height:350px;margin:20px 0;}
        .carousel-slide {padding:0 20px;flex-direction:column;text-align:center;justify-content:center;}
        .carousel-content h1{font-size:1.8rem;}
        .carousel-content p{font-size:1rem;}
        .carousel-nav {display:none;}
        .upload-btn{background-color:#555;padding:8px;border-radius:50%;}
      ...
      <main>
        <div className="container">
          <section className="hero-carousel">
            <div className="carousel-inner">
              {slides.map((slide, index) => (
                <div key={slide.id} className={`carousel-slide ${slide.background} ${index === currentSlide ? 'active' : ''}`}>
                  <div className="carousel-content">
                    <h1>{slide.title}</h1>
                    <p>{slide.description}</p>
                    <button className="cta-button" style={slide.buttonStyle}>{slide.buttonText}</button>
                  </div>
                </div>
              ))}
            </div>
            <div className="carousel-nav">
              <button className="carousel-prev" onClick={prevSlide}><i className="fas fa-chevron-left"></i></button>
              <button className="carousel-next" onClick={nextSlide}><i className="fas fa-chevron-right"></i></button>
            </div>
            <div className="carousel-controls">
              {slides.map((_, index) => (
                <div key={index} className={`carousel-dot ${index === currentSlide ? 'active' : ''}`} onClick={() => showSlide(index)}></div>
              ))}
            </div>
          </section>
          ...
          <section className="content-section">
              ...
              {activities.map((activity) => (
                <div key={activity.id} className="activity-item">
                  <div className="activity-header">
                    <img src="https://res.cloudinary.com/dqhawdcol/image/upload/v1758202400/e9ifs1tewfgemgxfc5kc.jpg" alt="Creator" className="creator-avatar" />
                    <div className="activity-info">
                      <div className="creator-name">{activity.creator}</div>
                      <div className="activity-time">{activity.time}</div>
                    </div>
                  </div>
                  <div className="playlist-thumbnail" style={{ background: activity.gradient }}>
                    <div className="play-button">
                      <i className="fas fa-play"></i>
                    </div>
                  </div>
                  <div className="playlist-info">
                    <div className="playlist-name">{activity.playlist}</div>
                  </div>
                  ...
                  <div className="comment-section">
                    <img src="https://res.cloudinary.com/dqhawdcol/image/upload/v1758202400/e9ifs1tewfgemgxfc5kc.jpg" alt="Commenter" className="commenter-avatar" />
                    <div className="comment-content">
                      <div className="commenter-name">{activity.comment.user}</div>
                      <div className="comment-text">{activity.comment.text}</div>
                      <div className="activity-time">{activity.comment.time}</div>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="trybe-events-container">
              <div className="trybe-container">
                <div className="section-header">
                  <h2>Trybes for you</h2>
                  <a href="#" className="view-all">View all</a>
                </div>
                <div className="trybe-list">
                  {trybes.map((trybe, index) => (
                    <div key={index} className="trybe-item">
                      <div className="trybe-name">{trybe}</div>
          ...
          {showPlaylist && (
            <div className="playlist-popup">
              ...
              {songs.map((song) => (
                <div 
                  key={song.id} 
                  className={`playlist-song-item ${currentSong?.id === song.id ? 'active' : ''}`}
